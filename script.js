...
function stopBGM() {
    if (audioElements.bgm) { audioElements.bgm.pause(); audioElements.bgm.currentTime = 0; }
}
function stopAllSounds() {
    Object.values(audioElements).forEach(el => { 
        if (!el) return; el.pause(); el.currentTime = 0; el.onended = null;
    });
}
...
function unlockAudioContext() {
    ...
    Object.values(audioElements).forEach(el => { 
        if (!el) return; el.play().then(() => { el.pause(); el.currentTime = 0; }).catch(() => {}); 
    });
...
async function playStageAudioSequence() {
    if (!isAudioUnlocked) return; 
    stopAllSounds();
    gameState = 'stage_start_anim'; 
    animationFrameCount = 0;
    await playSoundAndWait(audioElements.start); 
    if (gameState === 'stage_start_anim') { 
        gameState = 'playing';
        playSoundRobust(audioElements.bgm, true); 
    }
}
...
async function playAllClearSequence() {
    ...
    if (gameState === 'all_clear_part1') {
        gameState = 'all_clear_part2'; 
        playSoundRobust(audioElements.allClear);
    }
}
...
function playCaptureProceduralSound() {
    try {
        if (!isAudioUnlocked) return;
        if (audioElements.capture && audioElements.capture.readyState >= 2) {
            audioElements.capture.currentTime = 0; audioElements.capture.play().catch(()=>{}); return;
        }
        if (!audioContextProcedural) return;
        const osc = audioContextProcedural.createOscillator();
        const gain = audioContextProcedural.createGain();
        osc.type = 'square'; 
        osc.frequency.setValueAtTime(880, audioContextProcedural.currentTime); 
        osc.frequency.exponentialRampToValueAtTime(1760, audioContextProcedural.currentTime + 0.1); 
        gain.gain.setValueAtTime(0.5, audioContextProcedural.currentTime); 
        gain.gain.exponentialRampToValueAtTime(0.01, audioContextProcedural.currentTime + 0.15);
        osc.connect(gain); gain.connect(audioContextProcedural.destination);
        osc.start(); osc.stop(audioContextProcedural.currentTime + 0.2);
    } catch(e) {}
}
...
function playWhiteLineProceduralSound() {
    try {
        ...
        const gain = audioContextProcedural.createGain();
        osc.type = 'sawtooth'; 
        osc.frequency.setValueAtTime(600, audioContextProcedural.currentTime); 
        osc.frequency.exponentialRampToValueAtTime(100, audioContextProcedural.currentTime + 0.1); 
        gain.gain.setValueAtTime(0.2, audioContextProcedural.currentTime); 
        gain.gain.exponentialRampToValueAtTime(0.01, audioContextProcedural.currentTime + 0.1);
        osc.connect(gain); gain.connect(audioContextProcedural.destination);
        osc.start(); osc.stop(audioContextProcedural.currentTime + 0.1);
    } catch(e) {}
}
...
function playShockSound() {
    try {
        if (!isAudioUnlocked) return;
        if (audioElements.shock && audioElements.shock.readyState >= 2) {
            audioElements.shock.currentTime = 0; audioElements.shock.play().catch(()=>{}); return;
        }
        if (!audioContextProcedural) return;
        const osc = audioContextProcedural.createOscillator();
        const gain = audioContextProcedural.createGain();
        osc.type = 'square'; 
        osc.frequency.setValueAtTime(100, audioContextProcedural.currentTime); 
        osc.frequency.exponentialRampToValueAtTime(10, audioContextProcedural.currentTime + 0.3); 
        gain.gain.setValueAtTime(0.6, audioContextProcedural.currentTime); 
        gain.gain.exponentialRampToValueAtTime(0.01, audioContextProcedural.currentTime + 0.3);
        osc.connect(gain); gain.connect(audioContextProcedural.destination);
        osc.start(); osc.stop(audioContextProcedural.currentTime + 0.3);
    } catch(e) {}
}
...
function setupStateSoundSources() {
    const loadAudioSources = (id, filenameBase) => {
        ...
        mp3Source.src = `./assets/${filenameBase}.mp3`; mp3Source.type = 'audio/mpeg'; 
        audioEl.appendChild(mp3Source); audioEl.load();
    };
    loadAudioSources('audioStart', STATE_SOUNDS_CONFIG.start); 
    loadAudioSources('audioClear', STATE_SOUNDS_CONFIG.clear); 
    loadAudioSources('audioGameOver', STATE_SOUNDS_CONFIG.over); 
    loadAudioSources('audioAllClear', STATE_SOUNDS_CONFIG.all_clear);
    loadAudioSources('audioCapture', STATE_SOUNDS_CONFIG.capture);
    loadAudioSources('audioShock', STATE_SOUNDS_CONFIG.shock);
}
...
function processCapture() {
    ...
    for (let i = 0; i < player.path.length - 1; i++) {
        markLineOnGrid(player.path[i].x, player.path[i].y, player.path[i+1].x, player.path[i+1].y, 3);
    }
        ...
        outer: for(let r = 1; r <= searchRadius; r++) {
            for(let dy = -r; dy <= r; dy++) {
                for(let dx = -r; dx <= r; dx++) {
                    ...
                    if (nx >= 0 && nx < gridWidth && ny >= 0 && ny < gridHeight && grid[ny * gridWidth + nx] === 1) {
                        startX = nx; startY = ny; break outer;
                    }
                }
            }
        }
    ...
    if (startX !== -1) {
        ...
        let head = 0, tail = 0; q[tail++] = startX; q[tail++] = startY;
        grid[startY * gridWidth + startX] = 2;
        ...
        while (head < tail) {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                if (nx >= 0 && nx < gridWidth && ny >= 0 && ny < gridHeight && grid[ny * gridWidth + nx] === 1) {
                    grid[ny * gridWidth + nx] = 2; q[tail++] = nx; q[tail++] = ny;
                }
            }
        }
    }
    let currentPlayable = 0;
    let territoryCapturedSuccessfully = false; 
    for (let i = 0; i < grid.length; i++) {
        ...
        for (let x = 0; x < gridWidth; x++) {
            ...
            if (grid[idx] === 3 || grid[idx] === 4) {
                ...
                for (let oy = -1; oy <= 1; oy++) {
                    for (let ox = -1; ox <= 1; ox++) {
                        let nx = x + ox, ny = y + oy;
                        if (nx >= 0 && nx < gridWidth && ny >= 0 && ny < gridHeight && grid[ny * gridWidth + nx] === 0) isEdge = true;
                    }
                }
                grid[idx] = isEdge ? 3 : 4;
            }
    ...
    if (territoryCapturedSuccessfully) {
        playCaptureProceduralSound();
    } else if (lineDrawn) {
        playWhiteLineProceduralSound();
    }
    ...
    if (currentPercentage >= stages[currentStageIndex].goal) {
        if (currentStageIndex >= stages.length - 1) {
            ...
            gameState = 'clear'; 
            stopBGM(); 
            playSoundRobust(audioElements.clear);
        }
    }
}
...
function drawAnimatedImage(ctx, assetObj, charX, charY, charSize, currentDirection) {
    ...
    const imgIndex = Math.floor(animationFrameCount / 10) % assetObj.images.length;
    const img = assetObj.images[imgIndex];
    ctx.save();
    if (currentDirection === 'left') {
        ctx.scale(-1, 1); ctx.drawImage(img, -charX - charSize, charY, charSize, charSize);
    } else {
...
function drawEnemy() {
    ...
    if (!success) {
        ctx.beginPath(); ctx.arc(enemy.x, enemy.y, enemy.size / 2, 0, Math.PI * 2); ctx.fillStyle = enemy.color; ctx.fill(); ctx.closePath();
    }
}
...
function drawCoolText(text, x, y, baseFontSize, hasGradient = true, weight = 'bold', overrideColor = '#fff') {
    let fontFamily = getFontFamily(weight);
    let fontSize = baseFontSize;
    
    ctx.font = `${fontSize}px ${fontFamily}`;
    
    let textWidth = ctx.measureText(text).width;
    let maxWidth = gridWidth * 0.9;
    ...
    if (hasGradient) {
        ...
        gradient.addColorStop(0, '#fff'); gradient.addColorStop(0.5, '#ffd700'); gradient.addColorStop(1, '#ff8c00'); 
...
function gameLoop() {
    ...
    if (gameState === 'loading') {
        ...
        if (userLang === 'ja' && startLogoImg.complete && startLogoImg.naturalWidth !== 0) {
            let targetW = gridWidth * 0.8;
            let targetH = startLogoImg.height * (targetW / startLogoImg.width);
            ctx.save();
            ctx.globalAlpha = Math.min(1, animationFrameCount * 0.05); 
            ctx.drawImage(startLogoImg, (gridWidth - targetW)/2, (gridHeight - targetH)/2 - 50, targetW, targetH);
            ctx.restore();
        } else {
            drawCoolText(t.battleStart, gridWidth / 2, gridHeight / 2, 60);
        }
    }
    else if (gameState === 'playing') {
        ctx.drawImage(bgCanvas, 0, 0); 
        updatePlayer(); updateEnemy(); checkCollision();
        drawCharacters(); 
    } 
    else if (gameState === 'gameover_anim') {
        ctx.drawImage(bgCanvas, 0, 0);
        drawPlayer(); 
        let scale = 1 + animationFrameCount * 0.8; 
        ctx.save();
        ...
        ctx.strokeText(t.next, gridWidth / 2, gridHeight / 2 + 50);
        ctx.fillText(t.next, gridWidth / 2, gridHeight / 2 + 50);
    }
        ...
        ctx.strokeText(t.retry, gridWidth / 2, gridHeight / 2 + 50);
        ctx.fillText(t.retry, gridWidth / 2, gridHeight / 2 + 50);
    }
    else if (gameState === 'all_clear_part1' || gameState === 'all_clear_part2') {
        if (gameState === 'all_clear_part2' && endingImg.complete && endingImg.naturalWidth !== 0) {
            ctx.drawImage(endingImg, 0, 0, gridWidth, gridHeight);
        } else { 
            ctx.fillStyle = 'rgba(255, 215, 0, 0.4)'; ctx.fillRect(0, 0, gridWidth, gridHeight); 
        }
...
async function loadStage(index) {
    ...
    if (index >= MAX_STAGES) {
        gameState = 'all_clear'; stopAllSounds();
        playAllClearSequence();
        return;
    }
    ...
    if (!stageDef) {
        stages[index] = { goal: 75, enemySpeed: Math.min(8, 4 + Math.floor(index / 10)) };
        stageDef = stages[index];
    }
    
    gameState = 'loading';
    stopAllSounds(); 
    player.path = []; player.isDrawing = false;
    currentPercentage = 0; totalPlayablePixels = 0;
    ...
    await loadCharacterImagesRobust('player');
    let enemyId = getEnemyCharId(index);
    await loadCharacterImagesRobust('currentEnemy', enemyId);

    characterAssets.player.lastDirection = 'right';
    characterAssets.currentEnemy.lastDirection = 'right';

    player.x = gridWidth / 2;
    player.y = gridHeight - player.size; 
    
    enemy.x = gridWidth / 2;
    enemy.y = gridHeight / 2;

    if (index === 0) {
        enemy.speedX = stageDef.enemySpeed * (Math.random() > 0.5 ? 1 : -1);
        enemy.speedY = -stageDef.enemySpeed;
        enemy.size = 80; 
    } else if (index === 2) {
        let angle = Math.random() * Math.PI * 2;
        enemy.speedX = Math.cos(angle) * stageDef.enemySpeed;
        enemy.speedY = Math.sin(angle) * stageDef.enemySpeed;
        enemy.size = 192; 
    } else {
        enemy.speedX = stageDef.enemySpeed * (Math.random() > 0.5 ? 1 : -1);
        enemy.speedY = -stageDef.enemySpeed;
    ...
    await loadImageRobust(`./assets/bg_stage${numStr}.jpg`);
...
let isUIDragging = false, isDirectionInput = false;
let dragOffsetX = 0, dragOffsetY = 0;

function startUIDrag(e) { 
    isUIDragging = true; e.stopPropagation(); 
    ...
    let rect = dpadContainer.getBoundingClientRect();
...
function startDirectionInput(e) { 
    ...
    if (gameState === 'start_screen') { startGameFromScreen(); } 
    else { unlockAudioContext(); }
    let cx = e.type.includes('touch') ? e.touches[0].clientX : e.clientX;
    let cy = e.type.includes('touch') ? e.touches[0].clientY : e.clientY;
...
function handleUITouchMove(e) {
    if (isUIDragging || isDirectionInput) e.preventDefault();
    if(e.touches.length > 0) handleMoveGeneric(e.touches[0].clientX, e.touches[0].clientY);
}
function handleMoveGeneric(cx, cy) { 
    ...
    if (isUIDragging && dpadContainer) {
        ...
        dpadContainer.style.left = (cx - containerRect.left - dragOffsetX) + 'px';
        dpadContainer.style.top = (cy - containerRect.top - dragOffsetY) + 'px';
...
function updateDirection(cx, cy) { 
    ...
    if (isMovingLeft) characterAssets.player.lastDirection = 'left';
    else if (isMovingRight) characterAssets.player.lastDirection = 'right';
}
...
function updatePlayer() {
    ...
    player.x = Math.max(0, Math.min(gridWidth - player.size, player.x));
    player.y = Math.max(0, Math.min(gridHeight - player.size, player.y));
    let cx = Math.floor(player.x + player.size / 2);
    let cy = Math.floor(player.y + player.size / 2);

    let gridValue = grid[cy * gridWidth + cx];
    let isSafeGrid = (gridValue === 0 || gridValue === 3 || gridValue === 4);
    ...
    if (!isSafeZone) {
        if (!player.isDrawing) {
            ...
            player.path.push({ x: Math.floor(prevX + player.size / 2), y: Math.floor(prevY + player.size / 2) });
        }
        let last = player.path[player.path.length - 1];
        if (last.x !== cx || last.y !== cy) player.path.push({ x: cx, y: cy });
    } else {
        if (player.isDrawing) {
            player.path.push({ x: cx, y: cy }); processCapture(); player.isDrawing = false; player.path = [];
        }
    }
}

function updateEnemy() {
    if (enemy.speedX < 0) characterAssets.currentEnemy.lastDirection = 'left';
    else if (enemy.speedX > 0) characterAssets.currentEnemy.lastDirection = 'right';
    
    let currentDx = 0;

    if (currentStageIndex === 2) {
        let speedMag = stages[currentStageIndex].enemySpeed;
        let angleOffset = (Math.random() - 0.5) * 0.05; 
        let dx = (player.x + player.size/2) - enemy.x;
        let dy = (player.y + player.size/2) - enemy.y;
        let targetAngle = Math.atan2(dy, dx);
        let currentAngle = Math.atan2(enemy.speedY, enemy.speedX);
        
        let diff = targetAngle - currentAngle;
        while (diff > Math.PI) diff -= Math.PI * 2;
        while (diff < -Math.PI) diff += Math.PI * 2;
        
        angleOffset += Math.sign(diff) * Math.min(Math.abs(diff), 0.015);
        currentAngle += angleOffset;
        
        enemy.speedX = Math.cos(currentAngle) * speedMag;
        enemy.speedY = Math.sin(currentAngle) * speedMag;
        
        currentDx = enemy.speedX; 
    } else {
        currentDx = enemy.speedX; 
    }

    if (currentDx < 0) characterAssets.currentEnemy.lastDirection = 'left';
    else if (currentDx > 0) characterAssets.currentEnemy.lastDirection = 'right';

    let r = enemy.size / 2.5; 
    let cx = Math.floor(enemy.x); 
    let cy = Math.floor(enemy.y);
    ...
    if (currentStageIndex === 2) {
        ...
        for(let y = minY; y <= maxY; y++) {
            for(let x = minX; x <= maxX; x++) {
                ...
                if(dxx*dxx + dyy*dyy <= r*r) {
                    if(grid[y * gridWidth + x] === 4) {
                        eraseWhiteLine(x, y);
                        hitWhiteLine = true;
                    }
                }
    ...
    let checkX = Math.floor(enemy.x + enemy.speedX + (enemy.speedX > 0 ? r : -r));
    let checkY = Math.floor(enemy.y + enemy.speedY + (enemy.speedY > 0 ? r : -r));
    let isWall = (v) => v === 0 || v === 3 || v === 4; 
    ...
    if (!hitX && !hitY && checkX >= 0 && checkX < gridWidth && checkY >= 0 && checkY < gridHeight) {
        if (isWall(grid[checkY * gridWidth + checkX])) { hitX = true; hitY = true; }
    }

    if (hitX) enemy.speedX *= -1;
    if (hitY) enemy.speedY *= -1;
